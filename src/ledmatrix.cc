...
void LedMatrix::SetImageBuffer(const Nan::FunctionCallbackInfo<v8::Value> &args)
{
	...
	for (int x = 0; x < width; ++x)
	{
		...
		{
			int pos = y * 3 + x;
			matrix->SetPixel(x, y, buf[pos], buf[pos + 1], buf[pos + 2]);
		}
	}
...
void LedMatrix::Draw(const Nan::FunctionCallbackInfo<v8::Value> &args)
{
	...
	int starty = 0;
	if (args.Length() > 0 && args[0]->IsNumber())
		startx = Nan::To<int>(args[0]).FromJust();
	if (args.Length() > 1 && args[1]->IsNumber())
		starty = Nan::To<int>(args[1]).FromJust();
	...
	if (args.Length() > 2 && args[2]->IsNumber())
		width = Nan::To<int>(args[2]).FromJust();
	if (args.Length() > 3 && args[3]->IsNumber())
		height = Nan::To<int>(args[3]).FromJust();
	if (args.Length() > 4 && args[4]->IsNumber())
		imgx = Nan::To<int>(args[4]).FromJust();
	if (args.Length() > 5 && args[5]->IsNumber())
		imgy = Nan::To<int>(args[5]).FromJust();
	if (args.Length() > 6 && args[6]->IsBoolean())
		looph = Nan::To<bool>(args[6]).FromJust();
	if (args.Length() > 7 && args[7]->IsBoolean())
		loopv = Nan::To<bool>(args[7]).FromJust();
...
void LedMatrix::Scroll(const Nan::FunctionCallbackInfo<v8::Value> &args)
{
	...
	int starty = 0;
	if (args.Length() > 1 && args[1]->IsNumber())
		startx = Nan::To<int>(args[1]).FromJust();
	if (args.Length() > 2 && args[2]->IsNumber())
		starty = Nan::To<int>(args[2]).FromJust();
	...
	if (args.Length() > 3 && args[3]->IsNumber())
		width = Nan::To<int>(args[3]).FromJust();
	if (args.Length() > 4 && args[4]->IsNumber())
		height = Nan::To<int>(args[4]).FromJust();
	if (args.Length() > 5 && args[5]->IsNumber())
		scroll = Nan::To<int>(args[5]).FromJust();
	if (args.Length() > 6 && args[6]->IsNumber())
		speed = Nan::To<int>(args[6]).FromJust();
	if (args.Length() > 7 && args[7]->IsNumber())
		loop = Nan::To<int>(args[7]).FromJust();
